...
// NOTE: confirm this pin supports attachInterrupt on your Opta variant/expansion.
const int hallPin = 2;
...
#define VFD_RUN_CMD_REG    0x2000   // Control command word (run/stop)
...
void setup() {
  // USB serial for PC/Plotter/Python commands
  Serial.begin(115200);
  while (!Serial) delay(10);

  // Built-in RS485 port; library handles DE/RE switching automatically
...
void loop() {
  ...
  if (successX && successY && successZ) {
    ...
    Serial.println(", Fatg_Z(mm/s):"); Serial.println(fatigueZ, 3);
  }
}
...
// Writes control word 0x2000 to command the VFD to run/stop.
// Requires F0-02 = 2 ("Communication control") on the VFD keypad,
// otherwise this register is ignored.
// command: 0x0001 = Forward run, 0x0002 = Reverse run, 0x0005/0x0006 = stop variants
...
// or NAN if the read failed/timed out.
